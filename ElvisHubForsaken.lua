...
-- [90% English ; 10% Vietnamese on comments] --
...
local Window = Rayfield:CreateWindow({
   ...
   LoadingTitle = "Loading Elvis Hub...",
   LoadingSubtitle = "by Elvis",
   ShowText = "Elvis - Forsaken Script",
   Theme = "Default",
   ToggleUIKeybind = "K",
   ...
   Discord = {
      Enabled = true,
      Invite = "https://discord.gg/7zyT99D7S3",
      RememberJoins = true
   },
...
-- Periodic rescan to avoid errors
task.spawn(function()
    while true do
        task.wait(10)
        updateFakeNolis()
...
Aimbot:CreateDropdown({
    ...
    Flag = "AimModeSelect",
    Callback = function(Value)
        getgenv().AimbotConfig.Mode = Value[1] -- get first element
    end,
})

------------------------------------------------
-- Killers skill check function
...
-- Get nearest target by distance
...
-- Hidden function UI slow notification
...
-- General Anti-Slow handling function
...
-- Stop movement control variable
local stopMovement = false

-- Function to check the value of Result
local validValues = {
    Timeout = true,
    Collide = true,
    Hit = true
...
-- Listen when Player has Character
    ...
    if enable then
        if not getgenv().originalFootstepHook then
            getgenv().originalFootstepHook = hookmetamethod(game, "__namecall", function(self, ...)
                ...
                -- Block FootstepPlayed in UnreliableRemoteEvent
...
-- Create a common hook, block by remote name and args[1]
getgenv().setupRemoteHook = function(targetRemoteName, blockedFirstArg)
    getgenv()["savedHook_" .. targetRemoteName] = hookmetamethod(game, "__namecall", function(self, ...)
        ...
        -- Block if duplicate remote and args[1] is blockedFirstArg
...
-- I dont want to add these comments for each function anymore
...
-- Instances delete workspace aaaa
getgenv().workspaceTargets = {
    "GlitchParts"
}

-- Function delete instances Lighting aowk
...
-- I am the globglobglabgalab jk im tired of this
...
-- Maybe not, its longer then english
getgenv().initHook = function(targetRemote)
    getgenv().originalHookFunction = hookmetamethod(game, "__namecall", function(self, ...)
        ...
        if self == getgenv().MainRemote and getgenv().methodCalled == "FireServer" then
            -- 檢查遠端是否包含使用者名稱 using chinese got worse
...
local DEFAULT_WALK_SPEED = 16 -- toc do mac dinh
local ORIGINAL_DASH_SPEED = 50 -- toc do dash
local chargecontrol = true -- didnt you know i also speak Tieng Viet?
...
-- Helpers - im tired of this script
...
getgenv().attemptBlockForSound = function(sound)
    ...
    -- id early (string of digits) ---- HELP ME
...
-- Anim Match (i will prob use shorter comments)
...
-- Function store origi value player
...
Miscs:CreateToggle({
    ...
    Callback = function(Value)
        if Value then
            ...
            -- Reset Lighting and for the and uh eee oo ah
...
-- Rayfield Input to set delay dynamically
Miscs:CreateInput({
    Name = "Delay (seconds)",
...
getgenv().setupCharacter = function(char)
    ...
    -- Update animation speed according to WalkSpeed using Heartbeat
...
-- Track list (THANK YOU NYAN LIN OR WHATEVER FOR THIS)
getgenv().tracks = {
    ["None"] = "",
    ["----------- UST -----------"] = nil,
...
-- Use getgenv() to save global variables
...
Miscs:CreateToggle({
    ...
    Callback = function(value)
        ...
        if getgenv().chatEnabled then
            -- Connect Heartbeat to keep it on all the time
...
-- The function adjusts the position X scale by 0.39 once.
...
-- Remove background2, replace with transparent Frame to contain dropdown and Play button
