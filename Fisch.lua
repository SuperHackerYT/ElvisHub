...
function Notify(Des, Time, title)
	Fluent:Notify({
		Title= title or "Elvis Hub Notify",
		Content = Des,
		Duration = Time or 3
	})
end
...
_G['Elvis Hub Table'].SizeUi = (not _G['Elvis Hub Table'].isWindows and UDim2.fromOffset(600,300)) or UDim2.fromOffset(560,600)
if not _G['Elvis Hub Table'].isWindows then
...
do -- Config Function
	save = function()
		if not isfolder('Elvis Hub') then
			makefolder('Elvis Hub')
		end
...
do
	...
	InterfaceManager:SetFolder("Elvis Hub")
	InterfaceManager:BuildInterfaceSection(Tap.Settings)
	Window:SelectTab(1)
	SaveManager:LoadAutoloadConfig()
	Fluent:SetTheme("Elvis Theme")
