...
local Items = {
    "Tire", "Bolt", "Broken Fan", "Broken Microwave", "Shit Metal", "Old Radio", "Washing Machine", 
    "Old Car Engine", "Log", "Chair", "Coal", "Fuel Canister", "Oil barrel", "Cake", 
    "Cook Steak", "Cook Morsel", "Steak", "Morsel", "Berry", "Carrot", "Bandage", 
    "Medkit", "Revolver", "Rifle", "Leather Body", "Iron Body", "Revolve Ammo", "Rifle Ammo", "Giant Sack", "Good Sack", "Strong Axe", "Good Axe", "Gem"
}
...
pcall(function()
    Window:CreateTopbarButton("TransparencyToggle", "eye", function()
        ...
        print("Transparency state:", getgenv().TransparencyEnabled)
    end, 990)
end)
...
Tabs.Combat = Window:Tab({
    Title = "Combat",
    Icon = "sword",
    Desc = "Overpowered combat features"
})
...
Tabs.Auto = Window:Tab({
    Title = "Auto",
    Icon = "wrench",
    Desc = "Automatic Features"
})

Tabs.esp = Window:Tab({
    Title = "Esp",
    Icon = "sparkles",
    Desc = "Useful ESP features"
})
Tabs.br = Window:Tab({
    Title = "Bring",
    Icon = "package",
    Desc = "Bring items features"
})
Tabs.Tp = Window:Tab({
    Title = "Teleport",
    Icon = "map",
...
Tabs.Fly = Window:Tab({
    Title = "Player",
    Icon = "user",
    Desc = "Player adjustments"
})
Tabs.Vision= Window:Tab({
    Title = "Environment",
    Icon = "eye",
    Desc = "Environmental adjustment features"
})
...
Tabs.br:Section({ Title = "Bring", Icon = "box" })

Tabs.br:Dropdown({
    Title = "Select Items",
    Desc = "Choose which item to bring",
    Values = Items,
    Multi = false,
    AllowNone = true,
    Callback = function(option)
        selectedBringItem = option
    end
})
        ...
        if value then
            task.spawn(function()
                while bringItemsEnabled do
                    if selectedBringItem and humanoidRootPart then
                        ...
                        pcall(function()
                            ...
                            if itemsFolder then
                                for _, obj in pairs(itemsFolder:GetChildren()) do
                                    if obj.Name == selectedBringItem then
                                        table.insert(itemsToProcess, obj)
                                    end
                                end
                            end
                            
                            if characters then
                                for _, obj in pairs(characters:GetChildren()) do
                                    if obj.Name == selectedBringItem then
                                        table.insert(itemsToProcess, obj)
                                    end
                                end
                            ...
                            for _, item in pairs(itemsToProcess) do
                                if not bringItemsEnabled then break end
                        ...
                        if not selectedBringItem then
                            bringItemsEnabled = false
                            pcall(function()
                                Tabs.br:Find("Bring Items"):SetValue(false)
                            end)
                            break
                        end
                    end
